...
Badge color: green = closed issue, bench-verified on real hardware; grey
= not ported / no hardware to port to.

| Board | `HELM_FEATURE_CLI` | `HELM_HAS_ROM_BOOTLOADER_JUMP` | `HELM_HAS_DEBUG_LED` |
|---|---|---|---|
| `matek_h743` | [![bench-verified #1](https://img.shields.io/badge/bench--verified-%231-brightgreen)](https://github.com/heimdall-suite/heimdall-helm/issues/1) | [![bench-verified #1](https://img.shields.io/badge/bench--verified-%231-brightgreen)](https://github.com/heimdall-suite/heimdall-helm/issues/1)/[![#3](https://img.shields.io/badge/bench--verified-%233-brightgreen)](https://github.com/heimdall-suite/heimdall-helm/issues/3) <br />(USB DFU, `lib/bootloader/stm32h7.c`) | [![bench-verified #1](https://img.shields.io/badge/bench--verified-%231-brightgreen)](https://github.com/heimdall-suite/heimdall-helm/issues/1)/[![#3](https://img.shields.io/badge/bench--verified-%233-brightgreen)](https://github.com/heimdall-suite/heimdall-helm/issues/3) |
| `afroflight32` | [![bench-verified #13](https://img.shields.io/badge/bench--verified-%2313-brightgreen)](https://github.com/heimdall-suite/heimdall-helm/issues/13) | [![bench-verified #28](https://img.shields.io/badge/bench--verified-%2328-brightgreen)](https://github.com/heimdall-suite/heimdall-helm/issues/28) <br />(plain UART ROM bootloader, AN3155 protocol, `lib/bootloader/stm32f1.c`) | [![bench-verified #13](https://img.shields.io/badge/bench--verified-%2313-brightgreen)](https://github.com/heimdall-suite/heimdall-helm/issues/13) <br />(PB4 "CAL" LED) |
| `nexus_xr` | ![not ported](https://img.shields.io/badge/not_ported-lightgrey) | ![not ported](https://img.shields.io/badge/not_ported-lightgrey) | ![not ported](https://img.shields.io/badge/not_ported-lightgrey) |
...
| `diag mapping` | Dumps the mapping stage's own output directly (issue #34) — status, `pitch_mode` (`OFF`/`LIMIT`/`ACTIVE`, from a hardcoded placeholder channel, currently CH9), `pitch_target` (raw, from a placeholder channel, currently CH4). Not visible via `diag pipeline` yet: output.c still just copies `channels[]` through unchanged until #36 lands, so this is the only way to confirm those channels are actually being interpreted |
| `diag control` | Dumps the control stage's own output directly (issue #35) — status, `pitch_active` (`yes`/`no`; `no` when mapping's `pitch_mode` is `OFF`), `pitch_output` (placeholder passthrough of `pitch_target` when active, not real PID math). Same reasoning as `diag mapping`: output.c doesn't touch these fields yet, so this is the only way to confirm the Off/active branches both actually run |
...
| `param list` / `param show <name>` / `param set <name> <value>` | Only on boards with `HELM_FEATURE_PARAMS_PERSIST` set (`matek_h743`, `afroflight32` — see `lib/params/`, issue #32) — generic get/set/enumerate over the persisted-param store. `test_counter` (throwaway proof, issue #32), `input_mode` (real: `lib/rx/rx.c`'s runtime SBUS/CRSF pick, `0`/`1`, issue #10), and `servo_rate` (real: `lib/servo/servo.c`'s PWM frame rate, `0`=50Hz/`1`=250Hz/`3`=333Hz (`2` deliberately unassigned), issue #31 — one setting for every output slot, since period is a per-timer property shared across every channel on it; put servos with different rate ceilings on different timer groups if they need to run at different rates) all take effect on next boot only, not live. `input_mode`/`servo_rate` are read once at their module's own container-start; the 32 `out<N>.min`/`.center`/`.max`/`.reversed` params ([![pending verify #39](https://img.shields.io/badge/pending_verify-%2339-yellow)](https://github.com/heimdall-suite/heimdall-helm/issues/39), `N` = `output.c`'s own `slotConfigs[]` index, not the physical silkscreen label) are the exception: `output.c` reloads them from the param store every 20ms tick, so a `param set` here takes effect immediately, no reboot needed. On `matek_h743`, the same 32 params can also be set live over S.Port by a radio's Lua script ([![pending verify #42](https://img.shields.io/badge/pending_verify-%2342-yellow)](https://github.com/heimdall-suite/heimdall-helm/issues/42) `sportTelemetryPush()`) — `param set` remains the confirmed fallback if that doesn't land in time. Remaining consumers (function mapping, PID gains) are future work on top of this same interface |
