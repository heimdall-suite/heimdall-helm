...
# Selects exactly one USB CDC transport implementation per board
# (stm32h7.c, stm32f1.c) -- same explicit-source-selection reasoning as
...
# (lib/usb_cdc/<chip>.c, boards/<target>/usbd_conf.c/usbd_desc.c) is
...
#
# The middleware itself is per-chip-FAMILY, not per-board (framework-
# stm32cubeh7 vs framework-stm32cubef1 vs .../f7) -- custom_helm_
# stm32cube_package names the right one for this env, same explicit-
# config-over-auto-detection reasoning as every other custom_helm_*
# option here. Originally hardcoded to "framework-stm32cubeh7" (the only
# chip this had ever been ported to); that broke silently the moment a
# second chip family (#12, STM32F1) needed this same script.
platform = env.PioPlatform()
stm32cube_package = env.GetProjectOption("custom_helm_stm32cube_package")
